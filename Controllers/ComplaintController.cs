    ...
    public class ComplaintController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetComplaint(int systemID, int userID)
        {
            ...
            if(systemID > 0 && userID == 0) 
                {complaintCategoryList = await _dbContext.MasterComplaints.Where(u => u.SystemId == systemID).ToListAsync();}
            else if (userID > 0 && systemID == 0)
                {complaintCategoryList = await _dbContext.MasterComplaints.Where(u => u.UserId == userID).ToListAsync();}
            else
                { complaintCategoryList = await _dbContext.MasterComplaints.Where(u => u.UserId == userID).ToListAsync();}
